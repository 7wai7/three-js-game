...
function createDynamicBodyWithCollider(
  ...
  const rigidBody = physicsWorld.createRigidBody(
    RAPIER.RigidBodyDesc.dynamic().setTranslation(translation.x, translation.y, translation.z),
  );

  const collider = physicsWorld.createCollider(RAPIER.ColliderDesc.ball(0.5), rigidBody);
...
describe('physics components disposal', () => {
  ...
  it('disposes replaced real Rapier RigidBodies only after flush', () => {
    const previous = createDynamicBodyWithCollider(physicsWorld, { x: 0, y: 0, z: 0 });
    const next = createDynamicBodyWithCollider(physicsWorld, { x: 1, y: 0, z: 0 });
  ...
  it('recursively destroys world child entities and their real Rapier physics', () => {
    ...
    const rootPhysics = createDynamicBodyWithCollider(physicsWorld, { x: 0, y: 0, z: 0 });
    const childPhysics = createDynamicBodyWithCollider(physicsWorld, { x: 0, y: 1, z: 0 });
