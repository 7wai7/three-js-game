...
export default class PhysicsComponent extends MonoBehaviourComponent {
    ...
    constructor(rbDesc: RigidBodyDesc, colliderDesc: ColliderDesc) {
        ...
        this.rb = physicsWorld.createRigidBody(
            rbDesc
        );
    ...
    async init() {
        this.transformComponent = ecsService.getComponent(this.entityId, TransformComponent);
    }
    ...
    update(): void {
        ...
        this.transformComponent.position = { x: position.x, y: position.y, z: position.z };
        this.transformComponent.rotation = { x: rotation.x, y: rotation.y, z: rotation.z };
