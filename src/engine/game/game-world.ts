...
export default class GameWorld extends World {
  ...
  getGameObject(entity: EntityId): THREE.Object3D {
    ...
    if (!object) {
      throw new Error(`Game object not found for entity '${entity}'`);
    }
  ...
  getChildEntities(entity: EntityId, recursive = false): EntityId[] {
  ...
  getChildComponents<T extends Component>(
    ...
    for (const child of this.getChildEntities(entity, recursive)) {
      const component = this.getComponent(child, componentClass);
  ...
  getChildEntitiesFromObject(object: THREE.Object3D, recursive = false): EntityId[] {
  ...
  getParentComponent<T extends Component>(
    ...
    for (const parentEntity of this.getParentEntities(entity)) {
      const component = this.getComponent(parentEntity, componentClass);
