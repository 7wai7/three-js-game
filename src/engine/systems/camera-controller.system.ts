...
export default class CameraControllerSystem extends System {
  ...
  addPitch(delta: number) {
    ...
    this.pitchOffset = clamp(this.pitchOffset, -limit, limit);
  }
  ...
  private follow() {
    ...
    this.targetQuat.copy(this.yawQuat).multiply(this.pitchQuat);

    const t = 1 - Math.exp(-this.rotationSmoothness * this.dt);
    const slerpT = clamp(t, 0, 1);

    this.currentQuat.slerp(this.targetQuat, slerpT);

    this.worldOffset.copy(this.localOffset).applyQuaternion(this.currentQuat);

    this.desiredCameraPosition.copy(this.targetPosition).add(this.worldOffset);

    camera.position.copy(this.desiredCameraPosition);

    // look target
    this.targetLookPosition.copy(this.targetPosition).add(this.lookAtOffset);
